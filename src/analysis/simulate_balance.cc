...
int PendulumBalanceUp() {
  ...
  PendulumState<double> deviate_state;
  deviate_state.set_theta(0.1);
  deviate_state.set_thetadot(0.2);

  state.set_theta(desired_state.theta() + deviate_state.theta());
  state.set_thetadot(desired_state.thetadot() + deviate_state.thetadot());
  ...
  DRAKE_DEMAND(is_approx_equal_abstol(state.get_value(),
                                      ...
                                      std::cout<<"pendulum stabilized"<<std::endl;
  return 0;
}
...
int main(int argc, char* argv[]) {
  ...
  return drake::examples::pendulum::PendulumBalanceUp();
}

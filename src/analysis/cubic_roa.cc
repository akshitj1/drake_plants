...
void ComputeRegionOfAttraction()
{
    ...
    const double rho = 1.0 + 0.1;
    const Expression lambda{
        prog.NewSosPolynomial(Variables(xvar), 2).first.ToExpression()};
    //const Variable lambda{prog.NewContinuousVariables<1>("lambda").coeff(0)};
    //prog.AddLinearConstraint(lambda >= 0);
